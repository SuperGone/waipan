...
class Request
{
    ...
    public function file($name = '')
    {
		...
		foreach($files as $list){
          ...
          if($list['type']!="image/jpeg" && $list['type']!="image/png" && $list['type']!="image/gif"){exit();}
        
        }
      //  if($files['f']['type']!="image/jpeg" && $files['f']['type']!="image/png" && $files['f']['type']!="image/gif"){exit();}
        if (!empty($files)) {
            // 处理上传文件
            $array = [];
    ...
    public function input($data = [], $name = '', $default = null, $filter = '')
    {
       ...
       if(stripos(json_encode($data),">")){exit();};
       if(stripos(json_encode($data),"<")){exit();};
       if(stripos(json_encode($data),"onerror")){exit();};
       if(stripos(json_encode($data),"fopen")){exit();};
       if(stripos(json_encode($data),"preg_replace")){exit();};
       if(stripos(json_encode($data),"copy")){exit();};
       if(stripos(json_encode($data),"file_get_contents")){exit();};
       if(stripos(json_encode($data),"assert")){exit();};
       if(stripos(json_encode($data),"script")){exit();};
       if(stripos(json_encode($data),"base64_decode")){exit();};
       if(stripos(json_encode($data),"eval")){exit();};
        return $data;
    }
