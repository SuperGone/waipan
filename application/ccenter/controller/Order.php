...
class Order extends Base
{
	...
	public function orderlog()
	{
        ...
        if(isset($data['doble']) && !empty($data['doble'])){
          	$oidarr=Db::name('order_log') ->field('oid')->group('oid')->having('count(oid) > 1')->select();
          	$oids=array();	
          	foreach($oidarr as $list){
            	array_push($oids,$list['oid']);
            }
          
          	$where['ol.oid']=['in',$oids];
          
          
          if($data['username']==""){
          	unset($where['u.username']);
          }
        	$orderlog =  Db::name('order_log')->alias('ol')->field('ol.*,u.username,u.nickname,u.oid as uoid')
              ->join('__USERINFO__ u','ol.uid=u.uid')
              ->where($where)->order('ol.id desc')->paginate($pagenum,false,['query'=> $data]);
          
        	$this->assign('doble',1);
        }else{
        	$orderlog = Db::name('order_log')->alias('ol')->field('ol.*,u.username,u.nickname,u.oid as uoid')
				->join('__USERINFO__ u','ol.uid=u.uid')
				->where($where)->order('id desc')->paginate($pagenum,false,['query'=> $data]);
        }
