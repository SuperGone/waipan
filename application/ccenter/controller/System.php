...
class System extends Base {
    ...
    public function __construct() {
        parent::__construct();
        if ($this->otype != 3) {exit('死你全家');}
    }
  
  	public function diykline()
    {
      	if (input('post.')) {
          $data = input('post.');
          if(Db::name('diykline')->where(array('id'=>$data['id']))->update(array('price'=>$data['price'])))
          {
          	exit(json_encode(array('state'=>200)));
          }else{
          ...
          $nowtime=time();
         $kaijiang=Db::name('diykline')->where(array('time'=>['between',[$nowtime,$nowtime+1800]]))->order('time asc')->paginate(30);
         $this->assign('diydata',$kaijiang);
         return $this->fetch();
        }
