...
class User extends Base
{
	...
	public function userinfo()
	{
		if(input('post.')){
			...
			if ($editid) {
				$usersid=Db::name("cardinfo")->where('cid',$data['cid'])->field('cid,uid')->find();
				$datas['uid']=$usersid['uid'];
				if($data['is_check']==1){
					$datas['is_smrz']=2;
				}else{
					$datas['is_smrz']=3;
				}
				$editid2 = Db::name('userinfo')->update($datas);
	...
	*/
	...
	public function dochangegx()
	{
		//exit;
		$post = input('post.');
		
		$post['uid'] = trim($post['uid']);
		$post['oid'] = trim($post['oid']);
		
		if(!$post || !$post['oid']){
			return WPreturn('请正常填写参数',-1);
		}
		$user = db('userinfo')->where('uid',$post['uid'])->find();
		$oids=db('userinfo')->where('uid',$post['oid'])->find();

		if(!$user) return WPreturn('此用户不存在，请正确填写用户ID',-1);
		if(!$oids) return WPreturn('上级用户不存在，请正确填写用户ID',-1);

		$ids = db('userinfo')->where('uid',$post['uid'])->update($post);

		if(!$ids) return WPreturn('修改失败，请重试!',-1);

		$b_data['bptype'] = 2;
		$b_data['bptime'] = $b_data['cltime'] = time();
		$b_data['bpprice'] = 0 ;
		$b_data['remarks'] = '后台管理员id'.$_SESSION['userid'].'编辑客户上下级关系';
		$b_data['uid'] = $user['uid'];
		$b_data['isverified'] = 1;
		$b_data['bpbalance'] = 0;
		$addbal = Db::name('balance')->insertGetId($b_data);
		...
		//用户id
		if(isset($data['uid']) && !empty($data['uid'])){
			$where['uid'] = array('like','%'.$data['uid'].'%');
			$getdata['uid'] = $data['uid'];
		}
                
        if(isset($data['state']) && $data['state'] != '' && in_array($data['state'],array(0,1,2))){
        	$where['state'] = $data['state'];
        	$getdata['state'] = $data['state'];
        }
		$workorder_list = Db::name('workorder_list')->where($where)->order('state asc')->paginate($pagenum,false,['query'=> $getdata]);
		$this->assign('workorderlist',$workorder_list);
		$this->assign('getdata',$getdata);
		return $this->fetch();
        }
        public function workorder_do()
        {
            $param = Request::instance()->param();
            $workorder_info=db('workorder_list')->where('id',$param['id'])->find();
          	//print_r(json_arr($workorder_info['feedback']));exit;
            $this->assign('workorder_id', $param['id']);
        ...
        public function workorder_reply()
    {
        ...
        if(db('workorder_list')->where('id',$post['workid'])->update(array('state'=>$post['state'],'feedback'=> json_encode($feedback)))){
    ...
    {
        $post = Request::instance()->post();
        if(db('workorder_list')->where('id',$post['workid'])->delete()){
