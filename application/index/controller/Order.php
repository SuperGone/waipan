...
class Order extends Base
{
	...
	public function addorder()
	{
		...
		//用户信息
		$user = Db::name('userinfo')->field('ustatus,usermoney,uid,otype')->where('uid',$data['uid'])->find();
		//验证用户是否被冻结
		if($user['ustatus'] != 0){
        ...
        if($ids){
        	...
        	set_price_log($data['uid'],2,$u_fee,'下单','下单成功',$ids,$nowmoney);
          	shouxufanyong($fanli_shouxu,$data['uid'],$ids);
